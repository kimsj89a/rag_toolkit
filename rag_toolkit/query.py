"""
Query utilities and predefined query templates.
"""

from typing import List, Dict, Union, Any, Optional, Callable, TYPE_CHECKING, TypedDict
...
# ----- Predefined query templates for academic papers -----
...
# ----- General document overview queries -----
...
async def get_general_overview(
    ...
    """
    Get general overview of a document by querying predefined overview questions.

    Args:
        rag_client: RAG client instance
        mode: Query mode (default: "mix")
        max_section_length: Maximum length per section (0 = no truncation)
        clean_func: Optional function to clean/post-process each response
    """
    overview_parts = []
    ...
    for label, query in GENERAL_OVERVIEW_QUERIES.items():
        try:
            ...
            if response:
                ...
                if clean_func:
                    response_cleaned = clean_func(response_cleaned)
...
async def get_queries(
    ...
    return generate_general_queries(rag_client, overview, count)
